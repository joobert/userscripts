...
// @downloadURL  https://raw.githubusercontent.com/joobert/userscripts/main/scripts/YouTube%20Error%20Auto-Refresh.user.js
// @updateURL    https://raw.githubusercontent.com/joobert/userscripts/main/scripts/YouTube%20Error%20Auto-Refresh.user.js
// @license      MIT
...
(() => {
  ...
  function saveTimes(times) {
    try { localStorage.setItem(LS_KEY, JSON.stringify(times)); } catch {}
  }
  ...
  function* deepElements(root = document.documentElement) {
    ...
    while (q.length) {
      ...
      } else if (node.nodeType === 11) { // DocumentFragment (shadow root)
  ...
  function findErrorByTextDeep() {
    ...
    for (const el of deepElements()) {
      ...
      if (tag === "YT-FORMATTED-STRING" || el.id === "player-error-message-container" || el.classList?.contains("ytp-error")) {
        const t = (el.textContent || "").trim();
        if (!t) continue;

        if (t === ERROR_TEXT_EXACT) return { match: "exact", text: t, element: el };
        if (t.toLowerCase().includes(ERROR_TEXT_LOOSE)) return { match: "loose", text: t, element: el };
      }
    }
    return null;
  }
  ...
  function findPlayerOverlayError() {
    ...
    for (const el of candidates) {
      ...
      if (t.toLowerCase().includes(ERROR_TEXT_LOOSE)) return { match: "loose-ytp", text: t, element: el };
    }
    return null;
  }
