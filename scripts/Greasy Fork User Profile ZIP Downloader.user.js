...
// @downloadURL  https://raw.githubusercontent.com/joobert/userscripts/main/scripts/Greasy%20Fork%20User%20Profile%20ZIP%20Downloader.user.js
// @updateURL    https://raw.githubusercontent.com/joobert/userscripts/main/scripts/Greasy%20Fork%20User%20Profile%20ZIP%20Downloader.user.js
// @license      MIT
...
(async function () {
    ...
    function getMetaValue(code, key) {
        ...
        const jsMeta = code.match(
            new RegExp(`^\\/\\/\\s*@${escapedKey}\\s+(.+)$`, "m")
        );

        if (jsMeta) return jsMeta[1].trim();

        const cssMeta = code.match(
